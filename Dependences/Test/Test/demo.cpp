...
int main(int argc, char* argv[])
{
	...
	try {
		const char* filename = argv[1]; // 默认：C:\\Users\\iunin\\Downloads\\042002200311_57701788.pdf
...
void ExtractText(PdfMemDocument* pDocument, PdfPage* pPage)
{
	...
	//vector<map<double, string>> CommodityInfo; // element: Y Pos - info
					...
					strcmp(pszToken, "'") == 0)
				{
					...
					if (!pdfstr.IsHex()) // 排除发票数据的标题
					{
							 ...
							 138 <= dCurPosY && dCurPosY <= 224)
						{
							...
							prefix.append(std::to_string(CommodityIndex));
							prefix.push_back('/');
						}
								...
								(vecAreaCoor[2] <= dCurPosY && dCurPosY <= vecAreaCoor[3]))
							{
								...
								for (auto& data : mapData)
								{
										...
										(DataCoor[2] <= dCurPosY && dCurPosY <= DataCoor[3]))
									{
										...
										// 判断是否是Unicode编码
										if(0 == DataCoor[4])
											data = pdfstr.GetString();
										else if (1 == DataCoor[4])
										{
											data = UnicodeToString(tokenizer.m_UnicodeString);
											// 去除字符串里解码错误的问号
											for (size_t pos = data.find('?'); pos != string::npos;)
											{
												data.erase(pos, 1);
												pos = data.find('?');
											}
										}
										// 解析日期：2022年10月11日，index为偶数则不解析，奇数则解析Unicode
										...
										// 分析税额是否为"***"，若是则设置商品税率为"免税"
										else if (3 == DataCoor[4])
										{
											data = pdfstr.GetString();
											if ("***" == data)
											{
												string sl{ prefix };
												sl.at(prefix.size() - 1) = 'l';
												Pointer(sl.c_str()).Set(document, "免税");
											}
											// 税额后续为下一个商品，增加json商品索引
											++CommodityIndex;
										}

										// 判断json是否已存在，若存在则将数据附加到json中已存在数据后，使有多行数据的字段能够正确写入json
										if (nullptr == Pointer(prefix.c_str()).Get(document))
											Pointer(prefix.c_str()).Set(document, data.c_str());
										else
										{
